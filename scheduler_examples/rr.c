...
void rr_scheduler(uint32_t current_time_ms, queue_t *rq, pcb_t **cpu_task) {
    ...
    if (*cpu_task) {
        ...
        if ((*cpu_task)->ellapsed_time_ms >= (*cpu_task)->time_ms) {
            //notifica e liberta (igual ao original)
            msg_t msg = {
                .pid = (*cpu_task)->pid,
            ...
            *cpu_task = NULL;
            ...
            //actualiza contador RR se mudou a tarefa (ponteiro diferente) reinicia contador
    ...
    //Se CPU estiver livre, obtém próxima da ready-queue
    if (*cpu_task == NULL) {
        *cpu_task = dequeue_pcb(rq);
        if (*cpu_task) {
            rr_task_ptr = *cpu_task;
